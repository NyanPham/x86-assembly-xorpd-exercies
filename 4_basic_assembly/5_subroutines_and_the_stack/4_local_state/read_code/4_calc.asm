...
;
; Input:
;   Address of ASCII char 
; Output:
;   eax = integer between 0 and 9
; Operation:
;   Convert ASCII char to number by subtracting ASCII value by 30, and compare the bound 0 and 9
;
digit_to_num:
    .digit_addr = 8
    push    ebp
    ...
    sub     al,'0'									; subtract 30 to get integer value
    cmp     al,9
    jbe     .good_digit
...
;
; Input:
;   Address to the ASCII char
; Output:
;   eax = the subroutine address for *, + or -
; Operation:
;   Look up the correspoding subroutine for the operation char
;	
func_by_operator:
    .oper_addr = 8
    push    ebp
...
;
; Input:
;   Address of the expression string 
; Output:
;   eax = result of expression 
; Operation:
;   Parse the string from left to right in order of number1 -> operation -> number2
;	Convert number1 to int and store in ebx 
;	Get the corresponding oper subroutine and store it in edi 
; 	Conver the number2 to int and store it in eax 
; 	call edi, with arguments of ebx, eax
; 	return value in eax 
...
; Output:
;   eax = addition, subtraction or multiplication of a and b
; Operation:
;   add, subtract or multiply and b 
;	store and return result in eaax 
